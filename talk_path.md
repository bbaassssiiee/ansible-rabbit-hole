...
```
Layer 1: CONTENT
  skeletons/ + examples/ + documents/
  ↓
  "Here is the right way to do the thing"
...
The insight from Lada Kesseler's augmented-coding-patterns:
...
Make descriptions "pushy" (per the skill creator docs):
```
# Too passive:
description: Ansible best practices reference

# Pushy and trigger-rich:
description: >
  Deep Ansible expertise. Use automatically when the user writes, reviews,
  or asks about Ansible roles, playbooks, collections, inventory, or Molecule.
  Activate when you see YAML that looks like Ansible.
```
...
1. **The skeletons** — the domain's "right way to start"
   - Ansible: role-skeleton, collection-skeleton, project-skeleton
   - Terraform: module-skeleton, project-skeleton, workspace-skeleton
   - Kubernetes: namespace-skeleton, helm-skeleton, flux-skeleton

2. **The rules** — the domain's always-on best practices
   - Ansible: no_log on credentials, changed_when on commands
   - Terraform: remote state, pin providers, no hardcoded values
   - Kubernetes: resource limits, network policies, non-root containers

3. **The skills** — the domain expert persona
   - Change the skill descriptions to match the domain
   - Update the rabbit hole map (what leads to what in this domain)
   - Update the tone guidance if the audience is different
...
Pattern catalogs rot. Skills get stale. Skeletons diverge from current practice.
...
the engineer runs `/contribute` and documents it while it's fresh.
...
`/contribute` is the meta-command. Every pattern you add teaches the next engineer
who hits that same problem. The value of the repo grows with every contribution.

That's the meta-meta level: the repo is a collective memory of the team's
hard-won Ansible knowledge, surfaced by an AI coach that knows where all the holes are.
